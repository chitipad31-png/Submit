...
div[data-testid="stForm"] .stButton > button:hover {
    ...
    box-shadow:0 8px 24px rgba(0,61,124,0.35);
}

/* download */
.stDownloadButton > button {
    background:#fff !important; color:#003d7c !important;
    border:1.5px solid #003d7c !important; border-radius:10px !important;
    font-weight:700 !important; transition: background .15s !important;
}
.stDownloadButton > button:hover { background:#eef4ff !important; }
...
.form-card {
    ...
    box-shadow:0 4px 24px rgba(0,27,61,0.07); margin-bottom:20px;
...
with tab_form:
    ...
    # ── source อยู่นอก form เพื่อให้ช่องพิมเองขึ้นทันที ──
    sec("📖","ส่วนที่ 4 — หลักฐานอ้างอิง (เลือกก่อนกรอกฟอร์ม)")
    col_src, col_pg = st.columns([3,1])
    with col_src:
        source_choice = st.selectbox("แหล่งอ้างอิง *", SOURCES, key="src_select")
        source_custom = ""
        if source_choice == "อื่นๆ (พิมพ์เอง)":
            source_custom = st.text_input("ระบุแหล่งอ้างอิง", placeholder="Bloomberg, SEC Filing ...", key="src_custom")
    with col_pg:
        page_number_outside = st.text_input("เลขหน้า * (บังคับ)", placeholder="42", key="pg_outside")
    ...
    with st.form("submit_form", clear_on_submit=True):
        ...
        st.markdown('<div class="note-box">💡 ระบุตัวเลขให้ชัดเจน เช่น &quot;ยอดขายโต 69%&quot; หรือ &quot;ประหยัดต้นทุน 1.2 พันล้านยูโร&quot;</div>', unsafe_allow_html=True)
    ...
    if submitted:
        ...
        page_final   = page_number_outside.strip()
        errors = []
        ...
        if not page_final:             errors.append("เลขหน้า")
        if errors:
            ...
            insert_row({
                ...
                "source": source_final, "page_number": page_final,
            }, img_path)
            ...
            with btn1:
                if st.button("💾 บันทึกการแก้ไข", use_container_width=True):
                    supabase.table("submissions").update({
                        "full_name": new_name.strip(), "nickname": new_nick.strip(),
                        "student_id": new_sid.strip(), "team": new_team,
                        "key_findings": new_finding.strip(), "highlight_nums": new_nums.strip(),
                        "source": new_source.strip(), "page_number": new_page.strip(),
