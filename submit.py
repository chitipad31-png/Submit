...
with tab_form:
    ...
    # แต่ซ่อนไว้ใน session_state ก่อน แล้วค่อยใช้ตอน submit
    if "source_choice" not in st.session_state:
        st.session_state["source_choice"] = SOURCES[0]
    ...
    # ── ช่องพิมแหล่งอ้างอิงเอง อยู่นอก form ──
    source_custom = ""
    if source_choice == "อื่นๆ (พิมพ์เอง)":
        source_custom = st.text_input("✏️ ระบุแหล่งอ้างอิง", placeholder="Bloomberg, SEC Filing ...", key="src_custom_field")
